...
- 그런데 만약 자식 컴포넌트의 자식 컴포넌트에서 데이터를 필요로 한다면, 자식 컴포넌트가 중간에서 데이터를 전달해야 한다.
- 이 때, 자식 컴포넌트가 부모 컴포넌트의 데이터가 필요하지 않은 상호아이라면 불필요한 코드를 작성하게 된다.
- 컨텍스트는 데이터의 공급자와 소비자를 정의하고 데이터가 필요한 컴포넌트만 사용할 수 있게 구현할 수 있다.
---
### 077 - 컨텍스트로 부모 데이터 변경하기
- Props는 데이터가 부모에서 자식 컴포너늩로 단방향으로만 이동할 수 있다.
- 컨텍스트를 사용하면 자식 컴포넌트에서 부모 컴포너늩의 데이터를 변경할 수 있다.
---
---
---
---
---
### 078 - redux 리듀서로 스토어생성하기
...
- redux는 스토어-컴포넌트-액션-리듀서-스토어의 과정을 통해 변경한다.
```shell
...
---
### 080 - redux 스토어 상속과 디스패치 함수 실행하기
- 컴포넌트에서 dispatch 함수가 실행되면, 리듀서 함수는 액션 값을 참조해 작업을 실행힌다.
...
- react-redux의 장점은 크게 두 가지가 있다. 첫 번째는 store를 하위 컴포넌트에 매번 상속하지 않고 사용할 수 있다.
- 두 번째는 스토어 데이터를 사용, 변경하는 코드를 모듈화해 컴포넌트 내에 중복된 코드 사용을 최소화 할 수 있다.
```shell
> npm install --save redux
```
---
...
- 첫 번째 위치의 파라미터(mapStateToProps)는 스토어의 상태 값을 컴포넌트 props에 할당하는 함수이고, 두번째 파라미터(mapDispatchToProps)는 dispatch 함수를 컴포넌트 함수에 바인딩하는 함수다.
---
### 083 - react-redux로 스토어 데이터 변경하기
- connect 함수의 두 번째 라파미터 mapDispatchToProps 함수로, dispatch 함수를 컴포넌트 함수에 바인딩 할 수 있고, 컴포넌트 함수가 실행되면 바인딩된 dispatch 함수가 실행된다.
---
### 084 - redux 미들웨어 사용하기
